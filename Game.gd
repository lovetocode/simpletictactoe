...
func add_gameover_message() -> void:
	if is_draw:
		$GameOverMessagessage/Container/Label.text = "Game is a draw"
	else:
		$GameOverMessagessage/Container/Label.text = "Player " + player + " wins"
	$GameOverMessagessage.show()
